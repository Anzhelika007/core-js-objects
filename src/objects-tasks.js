...
function mergeObjects(objects) {
  const result = {};
  const arr = objects.map((x) => Object.entries(x)).flat();
  arr.forEach((x) => {
    const [key, value] = x;
 ...
 */
function makeWord(lettersObjec) {
  const result = [];
  Object.entries(lettersObjec).forEach((elem) => {
    const [value, position] = elem;
...
function sellTickets(queue) {
  ...
  const countTwentyFive = queue.filter((x) => x === 25).length;
  const countFiften = queue.filter((x) => x === 50).length;
  const sum = queue.reduce((acc, val) => {
    return acc + val;
  }, 0);
  if (
    sum >= 75 &&
    ((countFiften > 0 && countTwentyFive > 0) || countTwentyFive > 2)
  ) {
    return true;
  }
  return false;
...
function fromJSON(proto, json) {
  ...
  const result = Object.assign(newObj, JSON.parse(json));
  return result;
}
